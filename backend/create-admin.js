...
async function main() {
  ...
  await pool.query(
    `INSERT INTO users (email, password_hash, full_name, role, department)
     VALUES ($1, $2, 'Runtime Administrator', 'admin', 'Privacy Operations')
     ON CONFLICT (email) DO UPDATE SET password_hash=EXCLUDED.password_hash, full_name=EXCLUDED.full_name, role='admin', department=EXCLUDED.department`,
    [email, passwordHash]
  );
  await pool.end();
}
